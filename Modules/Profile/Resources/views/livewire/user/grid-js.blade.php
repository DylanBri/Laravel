<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', e => {
        $(document).on('app.ready', function () {
            ...
            App.Module.Profile.View.User.Grid = App.View.Grid.extend({
                el: "#userGrid",
                collection: new App.Collection.Pageable.Users(),
                btnSee: true,
                btnAdd: true,
                btnModify: true,
                btnDelete: false,
                ...
                renderSettings: function (id) {
                    var me = this;
                    if (me.data.user.settings === null) {
                        me.data.user.settings = new App.Module.Profile.View.Settings({
                            attributes : {
                                isModal: true,
                                parent: me,
                                type: 'user'
                    ...
                    me.data.user.settings.setId((id === null)? 0 : id);
                },

                renderAddress: function (id) {
                    var me = this;
                    if (me.data.user.address === null) {
                        me.data.user.address = new App.Module.Profile.View.Address({
                            attributes : {
                                isModal: true,
                                parent: me
                            }
                    ...
                    me.data.user.address.setId((id === null)? 0 : id);
                },

                renderModal: function (id, isNew, isModify) {
                    var me = this, txtSee = "<?php echo __('profile::user.See'); ?>",
                        txtAdd = "<?php echo __('profile::user.Add'); ?>",
                        txtModify = "<?php echo __('profile::user.Modify'); ?>";

                    if (me.data.modal === null) {
                        me.data.modal = new App.View.Component.Modal({
                            el : '#userModalContainer',
                            attributes: {
                                elModal : '#userModal'
                            }
                        });
                        ...
                        me.data.modal.$el.find(".btn-form-save").on('click', function () {
                            me.data.user.settings.formSubmit();
                            me.data.user.address.formSubmit();
                        });
                    ...
                    me.data.model = new App.Module.Profile.Model.User();
                ...
                saveModel: function () {
                    var me = this, token = me.data.user.address.$el.find('[name=_token]').val(),
                        userId = me.data.user.address.$el.find('#user_id').val(),
                        categoryId = me.data.user.address.$el.find('#category_id').val();
                    ...
                    if (me.data.stateSettings && me.data.stateAddress) {
                        ...
                        me.data.model.save({'_token': token, 'user_id': userId, 'category_id': categoryId})
                            .done(function (r) {
                                ...
                                me.data.alert.render();
                                Livewire.emit('user-form-success', r);
                                me.data.modal.$el.find(".btn-toggle-modal")[0].click();
                            })
                            .fail(function (r) {
                                var errors = r.responseJSON.errors;
                                Livewire.emit('user-form-error', errors);
                            });
                    }
                }
            });
            new App.Module.Profile.View.User.Grid();
        });
    });
</script>