<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', e => {
        $(document).on('app.ready', function () {
            ...
            if (App.Module.Profile.View.Manager === undefined) App.Module.Profile.View.Manager = {};
            App.Module.Profile.View.Manager.Grid = App.View.Grid.extend({
                el: "#managerGrid",
                collection: new App.Module.Profile.Collection.Pageable.Managers(),
                btnSee: true,
                btnAdd: true,
                btnModify: true,
                btnDelete: false,
                ...
                renderSettings: function (id) {
                    var me = this;
                    if (me.data.manager.settings === null) {
                        me.data.manager.settings = new App.Module.Profile.View.Settings({
                            attributes : {
                                isModal: true,
                                parent: me,
                                type: 'manager'
                    ...
                    me.data.manager.settings.setId((id === null)? 0 : id);
                },

                renderAddress: function (id) {
                    var me = this;
                    if (me.data.manager.address === null) {
                        me.data.manager.address = new App.Module.Profile.View.Address({
                            attributes : {
                                isModal: true,
                                parent: me
                            }
                    ...
                    me.data.manager.address.setId((id === null)? 0 : id);
                },

                renderModal: function (id, isNew, isModify) {
                    var me = this, txtSee = "<?php echo __('profile::manager.See'); ?>",
                        txtAdd = "<?php echo __('profile::manager.Add'); ?>",
                        txtModify = "<?php echo __('profile::manager.Modify'); ?>";

                    if (me.data.modal === null) {
                        me.data.modal = new App.View.Component.Modal({
                            el : '#managerModalContainer',
                            attributes: {
                                elModal : '#managerModal'
                            }
                        });
                        ...
                        me.data.modal.$el.find(".btn-form-save").on('click', function () {
                            me.data.manager.settings.formSubmit();
                            me.data.manager.address.formSubmit();
                        });
                    ...
                    me.data.model = new App.Module.Profile.Model.Manager();
                ...
                saveModel: function () {
                    var me = this, token = me.data.manager.address.$el.find('[name=_token]').val(),
                        userId = me.data.manager.address.$el.find('#user_id').val(),
                        categoryId = me.data.manager.address.$el.find('#category_id').val();
                    ...
                    if (me.data.stateSettings && me.data.stateAddress) {
                        ...
                        me.data.model.save({'_token': token, 'user_id': userId, 'category_id': categoryId})
                            .done(function (r) {
                                ...
                                Livewire.emit('manager-form-success', r);
                                me.data.modal.$el.find(".btn-toggle-modal")[0].click();
                            })
                            .fail(function (r) {
                                var errors = r.responseJSON.errors;
                                Livewire.emit('manager-form-error', errors);
                            });
                    }
                }
            });
            new App.Module.Profile.View.Manager.Grid();
        });
    });
</script>