<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', e => {
        $(document).on('app.ready', function () {
            ...
            if (App.Module.Profile.View.Supadm === undefined) App.Module.Profile.View.Supadm = {};
            App.Module.Profile.View.Supadm.Grid = App.View.Grid.extend({
                el: "#supadmGrid",
                collection: new App.Module.Profile.Collection.Pageable.SuperAdmins(),
                btnSee: true,
                btnAdd: true,
                btnModify: true,
                btnDelete: false,
                ...
                toggleModal: function () {
                    var me = this;
                    if (me.data.stateSettings && me.data.stateProfile && me.data.stateAddress) {
                ...
                renderSettings: function (id) {
                    var me = this;
                    if (me.data.supadm.settings === null) {
                        me.data.supadm.settings = new App.Module.Profile.View.Settings({
                            attributes : {
                                isModal: true,
                                parent: me,
                                type: 'supadm'
                    ...
                    me.data.supadm.settings.setId((id === null)? 0 : id);
                },

                renderAddress: function (id) {
                    var me = this;
                    if (me.data.supadm.address === null) {
                        me.data.supadm.address = new App.Module.Profile.View.Address({
                            attributes : {
                                isModal: true,
                                parent: me
                            }
                    ...
                    me.data.supadm.address.setId((id === null)? 0 : id);
                },

                renderModal: function (id, isNew, isModify) {
                    var me = this, txtSee = "<?php echo __('profile::supadm.See'); ?>",
                        txtAdd = "<?php echo __('profile::supadm.Add'); ?>",
                        txtModify = "<?php echo __('profile::supadm.Modify'); ?>";

                    if (me.data.modal === null) {
                        me.data.modal = new App.View.Component.Modal({
                            el : '#supadmModalContainer',
                            attributes: {
                                elModal : '#supadmModal'
                            }
                        });
                        ...
                        me.data.modal.$el.find(".btn-form-save").on('click', function () {
                            me.data.supadm.settings.formSubmit();
                            me.data.supadm.address.formSubmit();
                        });
                    ...
                    me.data.model = new App.Module.Profile.Model.SuperAdmin();
                ...
                saveModel: function () {
                    var me = this, token = me.data.supadm.address.$el.find('[name=_token]').val(),
                        userId = me.data.supadm.address.$el.find('#user_id').val(),
                        categoryId = me.data.supadm.address.$el.find('#category_id').val();
                    ...
                    if (me.data.stateSettings && me.data.stateAddress) {
                        ...
                        me.data.model.save({'_token': token, 'user_id': userId, 'category_id': categoryId})
                            .done(function (r) {
                                ...
                                Livewire.emit('supadm-form-success', r);
                                me.data.modal.$el.find(".btn-toggle-modal")[0].click();
                            })
                            .fail(function (r) {
                                var errors = r.responseJSON.errors;
                                Livewire.emit('supadm-form-error', errors);
                            });
                    }
                }
            });
            new App.Module.Profile.View.Supadm.Grid();
        });
    });
</script>